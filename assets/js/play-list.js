...
function simp_initTime() {
  ...
  if (simp_audio.currentTime == simp_audio.duration) {
    simp_controls.querySelector('.simp-plause').classList.remove('icon-pause');
    simp_controls.querySelector('.simp-plause').classList.add('icon-play');
...
function simp_setAlbum(index) {
  simp_cover.innerHTML = simp_a_url[index].dataset.cover ? '<div class="bg-img"></div>' : '<i class="icon-music fa-5x"></i>';
...
function simp_changeAudio(elem) {
  ...
  if (simp_isPlaying) {
    simp_controls.querySelector('.simp-plause').classList.remove('icon-play');
    simp_controls.querySelector('.simp-plause').classList.add('icon-pause');
  }


  // change the id  for the current audio
  var audioId = elem.dataset.id;

  simp_controls.parentElement.id = audioId;
  var audioMain= simp_controls.parentElement.querySelector('audio');

  // send episod info to back

  var dataSent = false;

  audioMain.addEventListener("timeupdate",()=>{
    var lisendTime=  parseInt(simp_controls.querySelector('.start-time').innerHTML.split(':')[0], 10);
    var allTime=  parseInt(simp_controls.querySelector('.end-time').innerHTML.split(':')[0], 10);


    if(lisendTime > allTime * 0.7  && !dataSent){


      // set audio id in local storage

if(!localStorage.getItem("local_"+simp_controls.parentElement.id)){
localStorage.setItem("local_"+simp_controls.parentElement.id, simp_controls.parentElement.id);

var dataToSend = {
  userId : podcast_page.getAttribute("data-user-id"),
  postId : podcast_page.getAttribute("data-podcast-id"),
  podcastName : simp_controls.parentElement.querySelector('.simp-title').innerHTML,
  allEpisode :  podcast_page.getAttribute("data-all-episode"),
  episodeId : elem.dataset.id,
  listenStatus : 1,
  listenTime : simp_controls.querySelector('.start-time').innerHTML,
  podcastTime : simp_controls.querySelector('.end-time').innerHTML,
};
...
xhr.onload = function () {
  if (xhr.readyState === 4 && xhr.status === 200) {
    ...
    console.error("خطا در ارسال درخواست");
  }
};
...
const audioInfo = new URLSearchParams();
audioInfo.append("action", "podcast_info");
audioInfo.append("_nonce", ajax_var.nonce);

for (const [key, value] of Object.entries(dataToSend)) {
  audioInfo.append(key, value);
}
...
function simp_startScript() {
  ...
  simp_source.forEach(function(item, index) {
    ...
    item.addEventListener('click', function() {
      ...
      if (!item.classList.contains('simp-active')){
        item.classList.add('simp-active');
      }
      simp_source.forEach(function(otherItem) {
        if (otherItem != item){
          otherItem.classList.remove('simp-active');
        }
  ...
  simp_controls.querySelector('.simp-plauseward').addEventListener('click', function(e) {
    ...
    if (eles.contains('simp-plause')) {
      if (simp_audio.paused) {
        ...
        simp_audio.play();
        simp_isPlaying = true;
        eles.remove('icon-play');
        eles.add('icon-pause');
      } else {
        simp_audio.pause();
        simp_isPlaying = false;
        eles.remove('icon-pause');
        eles.add('icon-play');
      }
    } else {
  ...
  simp_volume.addEventListener('click', function(e) {
    ...
    if (eles.contains('simp-mute')) {
      if (eles.contains('icon-volume')) {
        eles.remove('icon-volume');
        eles.add('icon-volume-slash');
        simp_v_slider.value = 0;
      } else {
        eles.remove('icon-volume-slash');
        eles.add('icon-volume');
        simp_v_slider.value = simp_v_num;
      }
    } else {
      simp_v_num = simp_v_slider.value;
      if (simp_v_num != 0) {
        simp_controls.querySelector('.simp-mute').classList.remove('icon-volume-slash');
        simp_controls.querySelector('.simp-mute').classList.add('icon-volume');
      }
    }
...
// Start simple player
const podcast_page= document.querySelector('.podcast-single');
const simpPlayList= document.querySelector('#simp');

if (simpPlayList && podcast_page) {
  ...
  simp_elem += '<div class="simp-display"><div class="simp-album w-full flex-wrap"><div class="simp-cover"><i class="icon-music fa-5x"></i></div><div class="simp-info"><div class="simp-title">Title</div><div class="simp-artist truncate-text-6">Artist</div></div></div></div>';
  simp_elem += '<div class="simp-controls flex-wrap flex-align">';
  simp_elem += '<div class="simp-plauseward flex flex-align"><button type="button" class="simp-prev icon-backward" disabled></button><button type="button" class="simp-plause icon-play" disabled></button><button type="button" class="simp-next icon-forward" disabled></button></div>';
  ...
  simp_elem += '<div class="simp-volume flex flex-align"><button type="button" class="simp-mute icon-volume"></button><input class="simp-v-slider" type="range" min="0" max="100" value="100"/></div>';
  simp_elem += '<div class="simp-others flex flex-align"><button type="button" class="simp-plext icon-play-circle" title="Auto Play"></button><button type="button" class="simp-random icon-random" title="Random"></button><div class="simp-shide"><button type="button" class="simp-shide-top icon-caret-up" title="Show/Hide Album"></button><button type="button" class="simp-shide-bottom icon-caret-down" title="Show/Hide Playlist"></button></div></div>';
